...
if (isset($_GET)) {
	if ($_GET['isAdd'] == 'true') {
		...
		$result = mysqli_query($link, "SELECT `question_id` , `question_name` , `question` , question.`expertise_id` , `expertise_name`, doctorname, `question_date` , answer_name, COUNT( answer_id )AS ans , answer_id
FROM `question`
LEFT JOIN answer
USING ( `question_id` )
LEFT JOIN user
